...
export class PaymentService implements IPaymentService {
  ...
  async processDepositPayment(
    ...
    if (entityType === "rental") {
      ...
      customerName = (booking.userId as { name?: string })?.name || booking.contactDetails?.name || "Unknown User";
  ...
  async processBalancePayment(
    ...
    if (entityType === "rental") {
      ...
      if (booking) {
        customerName =
          (booking.userId as { name?: string })?.name || booking.contactDetails?.name || "Unknown User";
        providerName = (booking.photographerId as { name?: string })?.name || "Unknown Photographer";
      }
    }
    ...
    if (entityType === "rental") {
      ...
      for (const oId of ownersToPay) {
        ...
        const itemForOwner = items.find(
          (it) =>
            String((it.ownerId as unknown as { _id?: mongoose.Types.ObjectId })._id || it.ownerId) === String(oId),
        );
        if (itemForOwner && itemForOwner.ownerId) {
          providerName = (itemForOwner.ownerId as unknown as {name?: string}).name || "Item Owner";
        }
      ...
      const customerName = (booking.userId as { name?: string })?.name || booking.contactDetails?.name || "";
