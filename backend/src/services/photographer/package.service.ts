import { IPackageService } from "./IPackageAvailabilityService";
import { IPackageRepository } from "../../repositories/interface/IPackageRepository";
...
export class PackageService implements IPackageService {
    private _repository: IPackageRepository;
    ...
    async createPackage(photographerId: string, data: CreatePackageDto): Promise<IBookingPackage> {
        // Validate inputs if needed (though DTO/Controller usually handles structure)
    ...
    async deletePackage(photographerId: string, packageId: string): Promise<boolean> {
        // Soft delete usually, or real delete?
        // Model has isActive, let's use that or add a status='DELETED'. 
        // Existing status field: "ACTIVE" | "INACTIVE"
    ...
    async getPhotographerPackages(photographerId: string): Promise<IBookingPackage[]> {
        // We might want to filter out DELETED ones
