...
export class HelpTopicRequestService implements IHelpTopicRequestService {
    ...
    async createRequest(data: Partial<IHelpTopicRequest>): Promise<IHelpTopicRequest> {
        ...
        if (existing) {
            const error = new Error("You have already suggested this topic. It is currently under review.");
    ...
    async updateRequestStatus(id: string, status: IHelpTopicRequest["status"]): Promise<IHelpTopicRequest> {
